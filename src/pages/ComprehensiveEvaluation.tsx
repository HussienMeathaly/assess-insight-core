...
const key = (d: string, e: string) => `${d}::${e}`;
...
export default function ComprehensiveEvaluation() {
  ...
  const toggle = (d: string, e: string) => {
    const k = key(d, e);
    setSelected((s) => (s.includes(k) ? s.filter((i) => i !== k) : [...s, k]));
  };
    ...
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <main className="container mx-auto max-w-3xl space-y-6 px-4 py-8">
        ...
        <div className="text-right">
          <h1 className="text-xl font-bold text-foreground sm:text-2xl">التقييم الشامل</h1>
          <p className="mt-2 text-sm leading-7 text-muted-foreground">
            اختر المحاور التي تريد تقييمها بشكل معمّق ثم اضغط إرسال.
          </p>
        </div>
        ...
        <div className="space-y-4">
          {lockedDomains.map((domain) => {
            const allKeys = domain.elements.map((e) => key(domain.name, e));
            ...
            return (
              <div key={domain.name} className="rounded-xl border border-border p-4">
                ...
                <button
                  ...
                  onClick={() => {
                    if (allSelected) {
                      setSelected((s) => s.filter((k) => !allKeys.includes(k)));
                    } else {
                ...
                <div className="mt-3 space-y-2 pr-10">
                  {domain.elements.map((element) => {
                    const isSelected = selected.includes(key(domain.name, element));
